...
# ============================================================
# 롤링 무중단 배포 스크립트 (stop-first)
#
# 사용법: bash deploy-stop-first.sh <이미지_태그>
# 예시:   bash deploy-stop-first.sh v1.0.1
#
# 동작 흐름 (3회 반복):
#   1. 구버전 컨테이너 1개 제거 (총 2개)
#   2. 신버전 컨테이너 1개 추가 (총 3개)
#   3. 신버전 컨테이너 헬스체크
#   4. 헬스체크 실패시 즉시 중단 (남은 구버전 유지)
#   5. 헬스체크 성공시 베이크 타임 대기 (기본: 20초)
# ============================================================
...
if [ $# -lt 1 ]; then
    ...
    echo "   사용법: bash deploy-stop-first.sh <이미지_태그>"
    echo "   예시:   bash deploy-stop-first.sh v1.0.1"
    exit 1
fi
...
# ============================================================
log "================================================================"
log "🔄 롤링 배포 시작 (stop-first) — 이미지 태그: ${IMAGE_TAG}"
log "================================================================"
log ""
...
for ROUND in 1 2 3; do
    log ""
    log "--- [${ROUND}회차 / 3회차] stop-first 교체 시작 ---"
    ...
    # 2. 신버전 컨테이너 1개 추가 (총 3개로 복구)
    log "🐳 신버전 컨테이너 1개 추가 (총 ${TOTAL_REPLICAS}개로 복구)..."
    (cd "$BASE_DIR" && bash "$SCRIPT_DIR/deploy-extra.sh" "$IMAGE_TAG" 0)

    # 3. 신버전 헬스체크 (ROUND개 healthy 기대)
    if ! health_check_new "$ROUND"; then
        log "❌ [${ROUND}회차] 헬스체크 실패 — 배포 중단 (남은 구버전 유지됨)"
        exit 1
    fi

    # 4. 베이크 타임 — 신버전이 실제 트래픽을 처리하는지 관찰
    log "⏱️  베이크 타임 ${BAKE_TIME}초 대기 중..."
    sleep "$BAKE_TIME"
...
log ""
log "================================================================"
log "🎉 롤링 배포 완료 (stop-first)!"
log "   신버전 ${TOTAL_REPLICAS}개 실행 중 | 이미지 태그: ${IMAGE_TAG}"
log "================================================================"
